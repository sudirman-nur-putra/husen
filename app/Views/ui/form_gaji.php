...
<body>
    <div class="card m-4">
        ...
        <div class="card-body">
            ...
            <form action="/pengeluaran/tambahgaji" method="post">
                <div class="form-outline mt-1 mb-3 w-100 bg-light border rounded-3">
                    <select name="namaadmin" class="form-select" aria-label="Default select example">
                        <?php foreach ($admin as $row) : ?>
                            <option value="<?= $row['id'] ?>" ?><?= $row['nama'] ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <h6 class="m-0">Tanggal Gaji</h6>
                <div class="form-outline mt-1 mb-3 w-100 bg-light border rounded-3">
                    <input required name="tanggal" type="date" id="birthday" class="form-control form-control-md">
                </div>
                <h6 class="m-0">nominal</h6>
                <div class="form-outline mt-1 mb-3 w-100 bg-light border rounded-3">
                    <input required name="nominal" type="text" class="form-control form-control-md" />
                </div>
